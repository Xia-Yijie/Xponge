    ...
    for typei, bondi in bonds.items():
        for atom_indexes in bondi:
            ...
            if len(residues) == 1:
                ...
                residue.type.Add_Bonded_Force(
                    BONDTYPE.entity(resatoms, BONDTYPE.types_different_name[bond_names[typei]]))
                ...
                for i in range(0, len(tempvar), 5):
                    if tempvar[i + 3] < 0:
                        if tempvar[i + 4] - 1 in impropers.keys():
                            impropers[tempvar[i + 4] - 1].append(
                                [tempvar[i] // 3, tempvar[i + 1] // 3, abs(tempvar[i + 2] // 3),
                                 abs(tempvar[i + 3]) // 3])
                        else:
                            impropers[tempvar[i + 4] - 1] = [
                                [tempvar[i] // 3, tempvar[i + 1] // 3, abs(tempvar[i + 2]) // 3,
                                 abs(tempvar[i + 3]) // 3]]
                    else:
                        if tempvar[i + 4] - 1 in propers.keys():
                            propers[tempvar[i + 4] - 1].append(
                                [tempvar[i] // 3, tempvar[i + 1] // 3, abs(tempvar[i + 2] // 3), tempvar[i + 3] // 3])
                        else:
                            propers[tempvar[i + 4] - 1] = [
                                [tempvar[i] // 3, tempvar[i + 1] // 3, abs(tempvar[i + 2] // 3), tempvar[i + 3] // 3]]
                        if tempvar[i + 2] > 0:
...
def parm7(filename):
    ...
    for i in range(atom_numbers):
        ...
        atom_i.residue.type._name2atom[atom_i.name].Extra_Exclude_Atoms(
            [atom_i.residue.type._name2atom[aton.name] for aton in atom_i.extra_excluded_atoms])
