...
def Impose_Angle(molecule, atom1, atom2, atom3, angle):
    ...
    crd[atom3_friends] = np.dot(crd[atom3_friends] - crd[molecule.atom_index[atom2]],
                                _get_rotate_matrix(np.cross(r12, r23), deltaAngle)) + crd[molecule.atom_index[atom2]]
    for atom in molecule.atoms:
        i = molecule.atom_index[atom]
        atom.x = crd[i][0]
        atom.y = crd[i][1]
...
def Impose_Dihedral(molecule, atom1, atom2, atom3, atom4, dihedral):
    ...
    crd[atom4_friends] = np.dot(crd[atom4_friends] - crd[molecule.atom_index[atom3]],
                                _get_rotate_matrix(r23, deltaAngle)) + crd[molecule.atom_index[atom3]]
    for atom in molecule.atoms:
        i = molecule.atom_index[atom]
        atom.x = crd[i][0]
        atom.y = crd[i][1]
...
def _link_residue_process_coordinate(molecule, atom1, atom2):
    ...
    for link_conditions in res.type.tail_link_conditions:
        ...
        if len(atoms) == 1:
            ...
            crd[atomB_friends] = np.dot(crd[atomB_friends] - crd[molecule.atom_index[atoms[1]]],
                                        _get_rotate_matrix(np.cross(rAO, rOB), deltaAngle)) + crd[
            ...
            crd[atomB_friends] = np.dot(crd[atomB_friends] - crd[molecule.atom_index[atoms[2]]],
                                        _get_rotate_matrix(rOA, deltaAngle)) + crd[molecule.atom_index[atoms[2]]]
    ...
    if resA.type.tail_next and resB.type.head_next:
        ...
        crd[atom2_friends] = np.dot(crd[atom2_friends] - crd[molecule.atom_index[atom2]],
                                    _get_rotate_matrix(rOA, deltaAngle)) + crd[molecule.atom_index[atom2]]
