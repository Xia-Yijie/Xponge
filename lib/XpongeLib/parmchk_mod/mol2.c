...
int rmol2(char *filename, int *atomnum, ATOM atom[], int *bondnum, BOND bond[],
          CONTROLINFO * cinfo, MOLINFO * minfo, int flag)
{
/*if flag =1, read in atom type, if flag ==0, do not read in atom type */
    ...
    char type[MAXCHAR];
    char tmpchar1[MAXCHAR], tmpchar2[MAXCHAR], tmpchar3[MAXCHAR], tmpchar4[MAXCHAR],
        tmpchar5[MAXCHAR];
    char tmpc1[MAXCHAR];
    char line[MAXCHAR];
    FILE *fpin;
    ...
    for (;;) {
        ...
        if (atomrecord) {
            strcpy(tmpchar5, "MOL");
            sscanf(line, "%d%s%lf%lf%lf%s%d%s%lf", &tmpint1, tmpc1, &tmpf1, &tmpf2,
                   &tmpf3, tmpchar4, &tmpint2, tmpchar5, &tmpf4);
            if (numatom == 0)
                mf1 = tmpint1;  /* atom number of first atom */
            if (overflow_flag == 0) {
                if (strlen(tmpc1) > 5) {
...
/* bond type is frozen during bond type assignment */
                    ...
                    if (strncmp(type, "ar", 2) == 0 || strncmp(type, "AR", 2) == 0) {
                        eprintf("You cannot freeze bond (%d-%d,%s)\n"
                                "since it is unclear whether it is a single or double bond.\n"
                                "You may change it to '7' (aromatic single) or"
                                " '8' (aromatic double).",
                                tmpint4 - mf1 + 1, tmpint5 - mf1 + 1, type);
                    }
                    if (strncmp(type, "un", 2) == 0 || strncmp(type, "UN", 2) == 0) {
                        eprintf("You cannot freeze bond (%d-%d,%s)\n"
                                "since the bond type unclear.",
                                tmpint4 - mf1 + 1, tmpint5 - mf1 + 1, type);
                    }
                }

                atom[tmpint4 - mf1].con[atom[tmpint4 - mf1].connum++] = tmpint5 - mf1;
                atom[tmpint5 - mf1].con[atom[tmpint5 - mf1].connum++] = tmpint4 - mf1;
                if (strcmp(type, "1") == 0)
                    itype = 1;
                    ...
                    printf("\nWarning: Unknown bond type for bond (%s-%s:%s).\n"
                             "         Assuming it is a single bond.\n",
                            atom[tmpint4 - mf1].name, atom[tmpint5 - mf1].name, type);
                    itype = 1;
                }
...
void wmol2(char *filename, int atomnum, ATOM atom[], int bondnum, BOND bond[],
           AROM arom[], CONTROLINFO cinfo, MOLINFO minfo)
{
    ...
    fprintf(fpout, "SMALL\n");
    if (strlen(cinfo.chargetype) <= 1)
        strcpy(cinfo.chargetype, "No Charge or Current Charge");
    fprintf(fpout, "%s\n\n\n", cinfo.chargetype);
    fprintf(fpout, "@<TRIPOS>ATOM\n");
    for (i = 0; i < atomnum; i++)
        fprintf(fpout, "%7d %-8s %10.4lf %10.4lf %10.4lf %-6s %5d %-8s %9.6lf\n", i + 1,
                atom[i].name, atom[i].x, atom[i].y, atom[i].z, atom[i].ambername,
                atom[i].resno, atom[i].aa, atom[i].charge);
    ...
    for (i = 0; i < bondnum; i++) {
        ...
        bondj = bond[i].bondj;
        if (bond[i].type == 1) {
            if (strcmp(atom[bondi].ambername, "N.am") == 0
                && strcmp(atom[bondj].ambername, "C.2") == 0) {
                flag = 0;
                for (j = 0; j < 3; j++)
                    if (atom[atom[bondj].con[j]].atomicnum == 8
                        && atom[atom[bondj].con[j]].connum == 1) {
                        fprintf(fpout, " %-4s\n", "am");
                        flag = 1;
                        break;
                    }
            ...
            if (strcmp(atom[bondj].ambername, "N.am") == 0
                && strcmp(atom[bondi].ambername, "C.2") == 0) {
                flag = 0;
                for (j = 0; j < 3; j++)
                    if (atom[atom[bondi].con[j]].atomicnum == 8
                        && atom[atom[bondi].con[j]].connum == 1) {
                        fprintf(fpout, " %-4s\n", "am");
                        flag = 1;
                        break;
                    }
        ...
        if (bond[i].type == 2) {
            if (strcmp(atom[bondi].ambername, "C.2") == 0
                && strcmp(atom[bondj].ambername, "O.co2") == 0)
                fprintf(fpout, " %-4s\n", "1");
            else if (strcmp(atom[bondj].ambername, "C.2") == 0
                     && strcmp(atom[bondi].ambername, "O.co2") == 0)
                fprintf(fpout, " %-4s\n", "1");
            else
                fprintf(fpout, " %-4s\n", "2");
            continue;
