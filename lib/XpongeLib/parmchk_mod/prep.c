...
int rprepi(char *filename, int *atomnum, ATOM * atom, int *bondnum, BOND * bond,
...
/*	char resname[MAXCHAR]; */
    ...
    i = 0;
    readindex = 0;
    atm = (ATOM *) emalloc(sizeof(ATOM) * ((*cinfo).maxatom + 3));
...
*/
        ...
        if (spaceline(line) == 1 && readindex == 4)
            readindex = -4;
        if (readindex == 1) {
            sscanf(line, "%d%s%s%s%d%d%d%lf%lf%lf%lf", &tmpint1, tmpchar1, tmpchar2,
                   tmpchar3, &tmpint2, &tmpint3, &tmpint4, &tmpfloat1, &tmpfloat2,
            ...
            if (overflow_flag == 0) {
                ...
                rotate0(atm[tmpint4 - 1], atm[tmpint3 - 1], atm[tmpint2 - 1], &atm[i - 8],
        ...
        if (readindex == 4) {
            ...
            if (suc_flag == 0) {
                eprintf("Invalid atom name (%s) in the LOOP section.",
                        tmpchar1);
            }
            for (k = 3; k < number; k++)
...
*/
        ...
        if (readindex == 1) {
            number++;
            if (overflow_flag == 0) {
                sscanf(line, "%d%s%s%s%lf%lf%lf%lf", &tmpint1, tmpchar1, tmpchar2,
                       tmpchar3, &tmpfloat1, &tmpfloat2, &tmpfloat3, &atm[i - 8].charge);
                strcpy(atm[i - 8].name, tmpchar1);
                strcpy(atm[i - 8].ambername, tmpchar2);
                atm[i - 8].x = tmpfloat1;
                atm[i - 8].y = tmpfloat2;
...
void wprep(char *filename, char *ifilename, int atomnum, ATOM * atom, int bondnum,
           BOND * bond, CONTROLINFO cinfo, MOLINFO * minfo, int index)
{
    char tmpchar[MAXCHAR];
    size_t copied_size;

    wac("ANTECHAMBER_PREP.AC0", atomnum, atom, bondnum, bond, cinfo, *minfo);
    esystem("cp -rf ANTECHAMBER_PREP.AC0 ANTECHAMBER_PREP.AC");
...
*/
    if (index == 1) {
        copied_size = build_exe_path(tmpchar, "prepgen", sizeof tmpchar, 1);
...
/*  esystem("rm -f ANTECHAMBER_PREP.AC");*/
    }
